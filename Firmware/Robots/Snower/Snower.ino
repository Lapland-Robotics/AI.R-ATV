// This define must be placed at the beginning before #include "ESP32_New_TimerInterrupt.h"
...
#define TIMER0_COUNT_FREQUENCY 1000000  // Timer counting frequency in microseconds
...
#define RCCHECK(fn) \
  { \
    ...
    if ((temp_rc != RCL_RET_OK)) { errorLoop(); } \
  }
...
// Speed publisher timer
bool IRAM_ATTR PublishSpeed(void* timerNo) {
  unsigned long currentTime = millis();
  unsigned long timeDuration;

  unsigned long pulsesLeft = 0;
  unsigned long leftLastPublishTime = 0;

  unsigned long pulsesRight = 0;
  unsigned long rightLastPublishTime = 0;
...
void setup() {
  ...
  pinMode(SpeedSensorLeftPin, INPUT_PULLUP);
  pinMode(SpeedSensorRightPin, INPUT_PULLUP);
  ...
  attachInterrupt(digitalPinToInterrupt(SpeedSensorLeftPin), speedLeft_interrupt, FALLING);
  attachInterrupt(digitalPinToInterrupt(SpeedSensorRightPin), speedRight_interrupt, FALLING);

  cmdVelDiffDrive = createCommandVelocity();

  microrosInit();  // microros initialize

  // Start timer0 for speed publishers
  ITimer0.attachInterruptInterval(TIMER0_COUNT_FREQUENCY / SPEED_PUBLISHER_FREQUENCY, PublishSpeed);
}
